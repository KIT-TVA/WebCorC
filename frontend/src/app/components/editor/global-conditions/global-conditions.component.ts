...
export class GlobalConditionsComponent {
    ...
    public askAi(index: number): void {
        this.aiChatService.addCondition(new Condition(this.items.at(index).value.name))
    }
    ...
    public importConditions(conditions: ICondition[]) {
        for (const condition of conditions) {
            const conditionControl = this._fb.group({
                name: new FormControl(condition.programStatement, [Validators.required])
            })

            this.items.push(conditionControl)
            this.treeService.addGlobalCondition(condition.programStatement)
        }
    }
