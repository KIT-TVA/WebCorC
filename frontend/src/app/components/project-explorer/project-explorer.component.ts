...
export class ProjectExplorerComponent {
  ...
  private _transformer = (element: ProjectElement, level: number): FlatNode => {
    ...
    const flatNode =
      existingNode && existingNode.path === element.path
        ? existingNode
        : new FlatNode(element, level);
    this.elementToNodeMap.set(element, flatNode);
    this.nodeToElementMap.set(flatNode, element);
    return flatNode;
  };

  private elementToNodeMap: Map<ProjectElement, FlatNode> = new Map<
    ProjectElement,
    FlatNode
  >();
  private nodeToElementMap: Map<FlatNode, ProjectElement> = new Map<
    FlatNode,
    ProjectElement
  >();
  ...
  private _treeFlatener = new MatTreeFlattener(
    this._transformer,
    (node) => node.level,
    (node) => node.expandable,
    (node) => (node as ProjectDirectory).content,
  );
  private _treeControl = new FlatTreeControl<FlatNode>(
    (node) => node.level,
    (node) => node.expandable,
  );
  private _dataSource = new MatTreeFlatDataSource(
    this.treeControl,
    this._treeFlatener,
  );
   ...
   */
  private flattenTree(
    element: ProjectElement = this.projectService.root,
  ): ProjectElement[] {
  ...
  public dropNode(event: CdkDragDrop<string[]>) {
    ...
    if (
      ...
      event.currentIndex >= flattendTree.length
    ) {
  ...
  public getRenameName(node: FlatNode) {
    return (this.nodeToElementMap.get(node) as RenameProjectElement)
      .elementName;
  }
