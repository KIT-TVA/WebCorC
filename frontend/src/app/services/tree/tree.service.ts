...
export class TreeService {
    ...
    private collectStatements(statement: IAbstractStatement | undefined, statements: IAbstractStatement[]) {
        if (statement && statements) {
            switch (statement.type) {
                ...
                case "COMPOSITION":
                    statements.push(statement)
                    this.collectStatements((statement as ICompositionStatement).firstStatement, statements)
                    this.collectStatements((statement as ICompositionStatement).secondStatement, statements)
                    return
                case "REPETITION":
                    statements.push(statement)
                    this.collectStatements((statement as IRepetitionStatement).loopStatement, statements);
                    return
                case "RETURN":
                    statements.push(statement);
                    return
                ...
                case "SELECTION":
                    statements.push(statement);
                    (statement as ISelectionStatement).commands.forEach(command => this.collectStatements(command, statements));
            }
        }
    }
    ...
    public get conditions(): Condition[] {
        ...
        this._globalConditions.forEach((condition) => conditionsArray.push(new Condition(condition)))
