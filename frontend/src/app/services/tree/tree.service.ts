...
export class TreeService {
    ...
    public collectSubtreeNodes(
        ...
        const nodes: AbstractStatementNode[] = [];
        for (const child of node.children) {
            if (child) {
    ...
    public collectAncestorNodes(
        node: AbstractStatementNode,
    ): AbstractStatementNode[] {
        const ancestors: AbstractStatementNode[] = [];
        let currentNode: AbstractStatementNode | undefined = node.parent;
        while (currentNode) {
    ...
    public markSubtreeUnverified(node: AbstractStatementNode): void {
        ...
        const ancestorNodes = this.collectAncestorNodes(node);
        ...
        ancestorNodes.forEach((ancestorNode) => {
            ancestorNode.statement.isProven = false;
            ancestorNode.statement.nodeState = 'unverified';
        })
        node.statement.isProven = false;
        node.statement.nodeState = 'unverified';
